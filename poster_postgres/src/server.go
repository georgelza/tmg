...
import (
	...
	"github.com/georgelza/tmg_poster_postgres/config"
...
func main() {
	...
	*/
	...
	grpcLog.Info("DB Name is\t", configuration.Database.DBName)
	grpcLog.Info("DB SSL Mode is\t", configuration.Database.SSLMode)
	...
	server.DBConn = database.SetupDBConnection(configuration.Database.DBName, configuration.Database.Username, configuration.Database.Password, configuration.Database.DBPort, configuration.Database.DBHost)
